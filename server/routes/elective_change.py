...
@changes_bp.route("/fetchchangerequests", methods=["POST"])
def fetchchangerequests():
    try:
        user = request.environ["user"]
        ...
        req = electiveChangeDB.find(
            {"semester": int(semester), "branch": branch}, {"_id": 0}
        )
        return {"requests": list(req)}, 200
    except:
        return {"error": "Authentication failed"}, 401
...
def changeelectives():
    try:
        ...
        semester = request.form["semester"]
        branch = request.form["branch"]
        changeRequests = electiveChangeDB.find(
            {"semester": int(semester), "branch": branch}, {"_id": 0}
        ).sort({"cpi": -1})
        electives_db = courses_db.find_one(
            {"semester": int(semester) + 1, "branch": branch}, {"_id": 0}
        )
        ...
        for doc in list(changeRequests):
            ...
            for choice in electives:
                for elective in choice:
                    if (
                        elective["code"] == toElective["code"]
                        and elective["name"] == toElective["name"]
                    ):
                        if elective["remaining_capacity"] > 0:
                            findTo = elective
                    if (
                        elective["code"] == fromElective["code"]
                        and elective["name"] == fromElective["name"]
                    ):
                        findFrom = elective
                if findTo != None and findFrom != None:
                    ...
                    student = regStatus_db.find_one(
                        {"roll_number": doc["roll_number"]}, {"_id": 0}
                    )
                    allotted = student["allotted_elective"]
                    for elective in allotted:
                        if (
                            elective["code"] == findFrom["code"]
                            and elective["name"] == findFrom["name"]
                        ):
